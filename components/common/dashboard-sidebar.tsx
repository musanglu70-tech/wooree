            ...
            <div key={group.label}>
              ...
              <button
                ...
                className={cn(
                  ...
                  (hasActive || isOpen) ? "text-white" : "text-[#9ca3af] hover:text-white hover:bg-white/5"
                )}
                style={(hasActive || isOpen) ? { background: "linear-gradient(90deg, #2563eb, #7c3aed)", boxShadow: "0 2px 8px rgba(37,99,235,0.35)" } : {}}
              >
                <span
                  className="h-2 w-2 rounded-full shrink-0"
                  style={{ background: (hasActive || isOpen) ? "rgba(255,255,255,0.8)" : "#374151" }}
                />
                <span className="flex-1 text-left">{group.label}</span>
                <ChevronDown
                  className={cn("size-4 shrink-0 transition-transform duration-200", isOpen && "-rotate-180")}
                  strokeWidth={2.5}
                  style={{ color: (hasActive || isOpen) ? "rgba(255,255,255,0.8)" : "#4b5563" }}
                />
              </button>
